    ...
    <div className="mb-6">
      ...
      <div className="flex gap-1.5">
        {Array.from({ length: total }).map((_, i) => (
          <div key={i} className={`h-1 flex-1 rounded-full transition-all ${i < current ? "bg-[#e8001c]" : "bg-gray-200"}`} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full bg-[#e8001c] hover:bg-[#c4001a] text-white font-bold py-4 rounded-full
      ...
      <button
        type="button"
        className="text-sm text-gray-700 underline underline-offset-2 text-center hover:text-gray-900 transition-colors"
      >
        Esqueci a senha
      </button>
      ...
      <div className="flex gap-3">
        {[
          ...
          <button
            ...
            type="button"
            aria-label={label}
            className="flex-1 border border-gray-300 rounded-full py-3 flex items-center justify-center
      ...
      <button
        ...
        className="w-full border border-gray-300 text-gray-700 hover:bg-gray-50 hover:border-gray-400
          font-semibold py-3.5 rounded-full transition-colors text-sm"
      >
        Entrar sem senha
      </button>
      ...
      <p className="text-sm text-gray-500 text-center">
        ...
        <button
          type="button"
          onClick={onRegister}
          className="font-bold text-gray-800 underline underline-offset-2 hover:text-[#e8001c] transition-colors"
        >
          Cadastre-se
        </button>
      </p>
    ...
    return (
      <form onSubmit={nextStep} className="flex flex-col gap-4">
        ...
        <div className="mb-2">
          ...
          <p className="text-sm text-gray-400 mt-1">Complete os dados e crie seu cadastro.</p>
        </div>
        ...
        <button
          type="submit"
          className="w-full bg-[#e8001c] hover:bg-[#c4001a] text-white font-bold py-4 rounded-full transition-colors text-sm"
        >
          Continuar
        </button>
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 transition-colors -mt-1 mb-1 w-fit"
      >
      ...
      <div className="mb-2">
        ...
        <p className="text-sm text-gray-400 mt-1">Escolha uma senha segura para sua conta.</p>
      </div>

      <DPasswordInput label="Senha (mín. 6 caracteres)" value={password} onChange={setPassword} />
      <DPasswordInput label="Confirmar senha" value={confirm} onChange={setConfirm} placeholder="Repita a senha" />
      ...
      <ul className="text-xs text-gray-400 space-y-1.5 pl-1">
        {[
          ...
          <li key={label} className={`flex items-center gap-2 transition-colors ${ok ? "text-green-600" : ""}`}>
            <div className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 transition-colors ${ok ? "bg-green-500" : "bg-gray-200"}`}>
              {ok && <svg className="w-2.5 h-2.5 text-white" viewBox="0 0 10 8" fill="none">
                <path d="M1 4l3 3 5-6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>}
            </div>
            {label}
